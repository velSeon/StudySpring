...
//advice + pointcut을 분리
	/*
	 * @Pointcut("execution(* get*(..))")//CALL이라는 함수가 실행할 경우//모든 리턴타입, 뒤에 ..은
	 * calld이란 함수가 실행될때 xxx함수가 관여하겠다. public void xxx() {} //실제 공통로직이 구현된 함수
	 * (ADVICE)-xxx()함수실행 등록
	 */
@Before("execution(* getInfo*(..))")//JoinPoint 공통로직이 실행될 시점, 
public void getInfo() {
